...
class UserService:
    ...
    def get_users_list(
        ...
        logger.info(f"Trying to get users with role(s): {roles}.")
        users_query = User.query.order_by(User.registration_timestamp)
        if roles:
            users_query = users_query.filter(User.roles.any(Role.name.in_(roles.split(","))))
        try:
            users = (
                users_query
    ...
    def get_users_count(self, roles: str = None) -> int:
        """Get users count"""
        logger.info(f"Trying to get users count with role(s): {roles}.")
    ...
    def get_users_by_ids(
        ...
        logger.info(f"Trying to get users with id(s): {user_ids}.")
        users_query = (
            User.query
            .filter(User.id.in_(user_ids))
            .order_by(User.registration_timestamp)
        )
        try:
            return [
                {
