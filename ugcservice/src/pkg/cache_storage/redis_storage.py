...
class RedisCacheService(ABSCacheStorage):
    ...
    async def delete_data(self, key: str):
        logger.info(f"Deleting data to redis cache with key: {key}")
        try:
            await self.redis.delete(key)
        except Exception:
            logger.exception("error while deleting data in redis")
        else:
            logger.info(f"successfully deleting from redis.")
